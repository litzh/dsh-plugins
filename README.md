...
| [dsh-beacon](dsh-beacon/) | 运行状态上报到 beacon 监控面板：进程/会话生命周期、任务开始/完成、危险命令、工具失败、等待确认（可 ACK）、异常与心跳。 |
...
- **dsh-beacon** — reports runtime status to a beacon monitoring panel: process/session lifecycle, task start/finish, dangerous commands, tool failures, confirmation waits (with ACK), errors, and heartbeats.
