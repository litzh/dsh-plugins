...
DSH Web 插件：按供应商/模型配置高峰计价时段。高峰期提交消息时在**对话窗口内提问**确认，长任务运行中进入高峰时在**下一个工具调用前**提问确认；支持继续 / 本次高峰不再提醒 / 暂停任务，状态栏常显当前高峰或低谷。
...
- 状态胶囊常驻右下角：`高峰 · provider/model` 或 `低谷 · provider/model`；进入/离开高峰时显示通知。
...
1. 在 `~/.dsh/peak-pricing.json` 配置一条全天规则（`"start": "00:00", "end": "00:00"`），重启 dsh web。
2. 打开任意会话并选中规则命中的模型：右下角显示 `高峰 · …`。
