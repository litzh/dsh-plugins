...
```bash
dsh plugin --profile web add git+ssh://git@github.com/litzh/dsh-plugins.git#v0.4.0&path:dsh-mcp-server
```
...
| 工具 | 说明 |
|---|---|
| `dsh_session_list` | 列出会话（id、标题、running、cwd、最近活动） |
| `dsh_session_search` | 全文搜索会话内容 |
| `dsh_session_create` | 新建会话（可选 cwd，默认宿主工作目录） |
| `dsh_prompt` | 注入文本 prompt，立即返回 `accepted`；省略 sessionId 则先建会话；用 `dsh_history` 轮询结果 |
| `dsh_history` | 读取会话最近消息（user/assistant/turn 结局），用于轮询 |
| `dsh_cancel` | 停止当前轮（排队消息保留） |
...
- `dsh_prompt` 是 fire-and-forget；长任务用 `dsh_history` 轮询，审批用 `dsh_approvals_list` / `dsh_approval_respond` 处理，否则卡在审批上的轮次不会推进。
